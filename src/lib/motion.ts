...
/**
 * チャットの再生（§6.12 / DECISION U-16 / U-44）
 *
 * 発言とスタンプが 1 手ずつ現れ、一巡したら間を置いて先頭から繰り返す。
 * 高さは最初から全行ぶん取り、未再生の行は不透明度だけを 0 にする — 1 行ずつ
 * 足すとセルが伸び縮みして隣のセルまで動く。
 * 畳む先は 0 行ではなく **1 行目を残した状態**（U-44）: 空のスレッドが 1 手ぶん
 * 見える瞬間は「何も表示されない」故障に見える。
 */
export const chatThread = {
  /** 1 手の間隔。900 は待たされて見えた（U-44）。600 で短い台詞を読み終え、点滅にはならない */
  stepMs: 600,
  /** 一巡後の間。最後の発言を読み切ってから畳む（step の約 3.3 倍） */
  holdMs: 2000,
